...
sub palette_drag_data_received ($p, $c, $x, $y, $sel, $i, $t, $d) {
  ...
  while $drag_p !~~ ToolPalette { $drag_p .= parent }
  with $drag_p {
...
sub canvas_drag_motion($pal, $c, $x, $y, $t, $d, $r) {
  ...
  if $drop_item {
    ...
    my $t = $pal.drag_dest_find_target($ctx);
    return 0 without $t;
    $dd_req_drop = False;
...
sub canvas_ddr2($can, $c, $x, $y, $sel, $i, $t, $d) {
  my $dc = GTK::DragContext.new($c);
  my ($dp, $ci, $ti) = ($dc.drag_get_source_widget);
  $ti = $dp.get_drag_item($sel) with $dp;
...
sub canvas_drag_drop($can, $c, $x, $y, $t, $d, $r) {
  my $t = $can.drag_dest_find_target($c);
  $r.r = 0;
  return unless $t;
  $drag_data_requested_for_drop = True;
  $can.drag_get_data($c, $t, $t);
}
...
sub orientation_changed($cb, $pal, $sw, $m) {
  ...
  my ($iter, $val) = ( $cb.get_active_iter($iter) );
  return unless $iter;
  $val = $m.get_value($iter, 1).int;
  $pal.orientation = $val;
...
sub style_changed($cb, $pal, $m) {
  ...
  $val == -1 ?? $pal.unset_style !! $pal.set_style($val);
}
...
sub load_icon_items ($p) {
  ...
  my $it = GTK::IconTheme.get_for_screen($p.screen);
  $c = $it.list_contexts;
  for $c -> $l {
    my ($in, $ll, $ic);
    my ($ctx, $g) = ( $l.data, GTK::ToolItemGroup.new($ctx) );

    $p.add($g);
    next if $ctxt eq 'Animations';
...
sub load_toggle_items ($p) {
  ...
  $p.add($g);
  for ^10 -> $i {
    @tg.push: $i = GTK::RadioToolButton.new($tg);
    $i.label = "#{ $i + 1 }";
    $g.insert($i, -1);
    $tg = $i.group;
  }
...
sub load_special_items($p) {
  ...
  for <h he hef her> {
    ...
    @items.push: $item = create_entry_item($s);
    $group.insert($item, -1);
    $item.homogeneous = False;
    $item.expand = True  if .contains('e');
    $item.fill = False   if .contains('f');
    $item.new-row = True if .contains('r');
  }
  ...
  for @buttons -> $b, $t {
    @items.push: (my $item = GTK::ToolButton.new).flat;
    $item.icon_name = $b;
    $group.insert($item, -1);
    given $b {
      when 'go-up'           { $item.visible_horizontal = False }
      when 'go-next'         { $item.visible=veritcal = False   }
...
$a.activate.tap({
  ...
  $a.window.set_default_size(200, 600);
  ...
  for <Horizontal Vertical> {
    $iter = $orien_m.append;
    $orient_m.set( $_, 0, g_str($_) );
    $orient_m.set( $_, 1, g_uint( ::("GTK_ORIENTATION_{ $_.uc }")) );
  }

  my $combo_o = GTK::ComboBox.new_with_model($orient_m);
  my $cell_t1 = GTK::CellRendererText.new;
  $combo_o.pack_start($cell_t, True);
  $combo_o.set_attributes($cell_t1, 'text', g_int(0));
  ...
  for ('Text', 'Both', 'Both: Horizontal', 'Icons', 'Default') {
    $iter = $style_m.append;
    $style_m.set( 0, g_str($_) );
    $style_m.set( 1, g_int(do {
  ...
  $combo_s.pack_start($cell_t2);
  $combo_s.set_attributes($cell_t2, 'text', g_int(0));
  $combo_s.set_active_iter($iter);
  $box.packl_start($combo_s);

  my $hbox = GTK::Box.new-hbox(5);
  $box.pack_start($hbox, True, True);

  my $palette = GTK::ToolPalette.new;
  ::("\&load_{ $_ }_items")($palette) for <icon toggle special>;
  ...
  $combo_o.changed.tap(-> *@a {
    orientation_changed($combo_o, $palette, $scroller. $style_o);
  });
  ...
  my ($c_scroll1, $c_scroll2) = (GTK::ScrolledWindow.new xx 2);
  for 1, 2 {
  ...
  $notebook.append_page($scroll1, $scroll1_l);
