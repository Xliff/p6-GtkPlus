...
sub MAIN (
   $dir      is copy  = %config<include-directory>,
  :$file,
  :$only     is copy,
  :$exclude
) {
  my (%enums, @files, %etype, %values);

  $only = $only.split(',').cache if $only;
  $dir  = $dir.split(',').Array  if $dir.contains(',');
  $dir  .= Array unless $dir ~~ Array;
  ...
  for $dir[] -> $d {
    ...
    die 'Cannot specify a directory if using --file'
      if $d.defined && $file.defined;

    if $file.defined {
      die "File '$file' does not exist" unless $file.IO.e;
      @files = ($file);
    } else {
      die "Directory '$d' either does not exist, or is not a directory"
        unless $d.IO.e && $d.IO.d;

      @files = find-files( $d, :$exclude, extension => 'h' );
    }
